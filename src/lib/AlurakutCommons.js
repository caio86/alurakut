...
export function OrkutNostalgicIconSet(props) {
  return (
    <OrkutNostalgicIconSet.List>
      {[
        { name: 'Recados', slug: 'recados', icon: 'book' , value: 4},
        { name: 'Fotos', slug: 'fotos', icon: 'camera' , value: 3},
        { name: 'Videos', slug: 'videos', icon: 'video-camera' , value: 0},
        { name: 'Fãs', slug: 'fas', icon: 'star' , value: -1},
        { name: 'Mensagens', slug: 'mensagens', icon: 'email' , value: 2},
      ].map(({ name, slug, icon , value}) => (
        <li key={`orkut__icon_set__${slug}`}>
          ...
          <span className="OrkutNostalgicIconSet__number" style={{ gridArea: 'number' }}>
            ...
            {props[slug] ? props[slug] : value}
          </span>
        </li>
      ))}
      {[
        { name: 'Confiável', slug: 'confiavel', icon: 'smile', iconesAtivos: 3},
        { name: 'Legal', slug: 'legal', icon: 'cool', iconesAtivos: 2},
        { name: 'Sexy', slug: 'sexy', icon: 'heart', iconesAtivos: 1},
      ].map(({ name, slug, icon , iconesAtivos}) => {
        const total = props[slug] ? props[slug] : iconesAtivos;
        return (
          <li key={`orkut__icon_set__${slug}`}>
